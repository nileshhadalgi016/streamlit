...
To protect reviewer bandwidth and keep the process fair for active contributors, maintainers may take stricter action when a contributor repeatedly submits low-quality or non-responsive PRs. This may include deprioritizing reviews, requesting that existing feedback be resolved first, or closing newer PRs. For repeated non-responsive submission patterns, maintainers may apply stricter prioritization and moderation to non-core contributors. When necessary, we also reserve the right to use GitHub moderation tools to protect the project and community, up to and including banning users in severe or repeated abuse cases.
